(() => {
            ...
            <div class="detail">
                ...
                <div class="xl temperature">
                    <span>${day.temp.day.toFixed()}<span class="current-unit">°</span><span class="night-temp"><span
                                class="slash standard">/${day.temp.min.toString().startsWith('-') ? '' : ' '}</span>${day.temp.min.toFixed()}<span class="current-unit">°</span></span></span>
    ...
    /**
     * Renders location options and sets up event listeners for location selection.
     *
     * @param {Array} locationOptionsData - An array of location data objects to be rendered as options.
     */
    ...
    /**
     * Generates HTML content for location options based on provided location data.
     *
     * @param {Array} locationOptionsData - An array of location objects.
     * @returns {string} - A string containing HTML for the location options.
     */
    const renderLocationOptionsContent = (locationOptionsData) => {
        ...
        locationOptionsData.forEach((location) => {
            const locationDescription = `${location.name}, ${location.state ? location.state + ', ' : ''}${location.country}`;
            const lat = location.lat;
            const lon = location.lon;
            if (!uniqueLocations.has(locationDescription)) {
        ...
        const html = `
            <div class="center">
                <div class="loader-wrapper visible">
                    <div id="location-loader" class="loader visible"></div>
                </div>
            </div>
        `;
        if ($('#location-loader').length === 0) {
            locationHistory.empty().append(html);
        }
    };
    ...
    $(document).on('click', (e) => {
        if (!$(e.target).closest('input, #clear-icon, #location-icon').length) {
